...
describe("initSessionState conflict retry wiring", () => {
  it("retries production conflicts until the competing writer settles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-session-conflict-retry-"));
