...
export async function runWithSessionInitConflictRetry<T>(
  ...
  for (let attemptNumber = 1; attemptNumber <= maxAttempts; attemptNumber += 1) {
    try {
      ...
      const backoffMs = computeBackoff(SESSION_INIT_CONFLICT_BACKOFF_POLICY, attemptNumber);
      log.debug(
        `reply session initialization conflicted; retrying in ${backoffMs}ms (attempt ${attemptNumber + 1}/${maxAttempts})`,
      );
  ...
  throw new Error("reply session initialization retry loop exhausted");
}
